...
namespace GoogleMobileAds.Editor
{
    public class EditorLocalization
    {
        ...
        private EditorLocalizationData GetLocalizationData()
        {
            return _localizationData.Value;
        }
        ...
        public string ForKey(string key)
        {
                    ...
                    out var localizations))
            {
                ...
                var userLanguage = GoogleMobileAdsSettings.LoadInstance().UserLanguage;
                if (localizations == null) return null;
                var userLanguageExists = localizations.TryGetValue(userLanguage,
                    out var userLocalization);
                var userLocalizationIsValid = userLanguageExists &&
        ...
        private static EditorLocalizationData InitLocalizationDataOrThrow()
        {
            ...
            try
            {
                var json = File.ReadAllText(localizationDataPath);
                var data = DeserializeFromJson(json);
                if (data.LocalizationsByKey == null) throw new ArgumentNullException("LocalizationsByKey");
                return data;
            }
            catch (Exception)
            {
