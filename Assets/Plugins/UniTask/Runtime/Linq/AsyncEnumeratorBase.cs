        ...
        IUniTaskAsyncEnumerator<TResult>
    {
        ...
        protected void SourceMoveNext()
        {
            ...
            if (sourceMoveNext.IsCompleted)
            {
                ...
                try
                {
                    var (waitCallback, requireNextIteration) = TryMoveNextCore(sourceMoveNext.GetResult(), out result);

                    if (waitCallback) return;

                    if (requireNextIteration) goto CONTINUE;
        ...
        private static void MoveNextCallBack(object state)
        {
            ...
            try
            {
                var (waitCallback, requireNextIteration) =
